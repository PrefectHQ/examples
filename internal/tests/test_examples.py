...
def find_example_files(directory: str = 'examples', extensions: List[str] = ['.py']) -> List[str]:
    ...
    for root, _, files in os.walk(directory):
        ...
        for file in files:
            if any(file.endswith(ext) for ext in extensions) and not file.startswith('__'):
...
def main():
    ...
    parser.add_argument(
        "files", 
        nargs="*", 
        help="Specific example files to test"
    )
    parser.add_argument(
        "--all", 
        action="store_true", 
        help="Test all examples in the repository"
    )
    parser.add_argument(
        "--dir", 
        default="examples", 
    ...
    if not files:
        print("No files specified. Use --all to test all examples or provide specific files.")
        return 1
